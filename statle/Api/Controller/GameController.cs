...
public class GameController : ControllerBase
{
    ...
    public IActionResult GuessStat(string stat)
    {
        ...
        if (currentGame == null || currentPokemon == null)
        {
            return BadRequest("Failed to load game from session.");
        }
    ...
    public async Task<IActionResult> SaveGame()
    {
        ...
        if (string.IsNullOrEmpty(currentGameJson))
            return BadRequest("No game to save.");
        ...
        var game = new Game
        {
            ...
            UsedStatsJson = System.Text.Json.JsonSerializer.Serialize(currentGame.UsedStats)
        };
        ...
        HttpContext.Session.Remove("CurrentPokemon");
